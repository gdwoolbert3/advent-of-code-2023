defmodule AdventOfCode2023.Day2.Problem1 do
  @moduledoc """
  ...
  defp process_game(game) do
    game = String.replace(game, " ", "")
    ["Game" <> id, game] = String.split(game, ":")
    if is_valid?(game), do: to_num(id), else: 0
  end

  defp is_valid?(game) do
    game
    |> String.split([",", ";", "\n"], trim: true)
    |> Enum.map(&Integer.parse/1)
    |> Enum.reduce_while(true, fn {count, color}, _ ->
      if is_valid_draw?(color, count), do: {:cont, true}, else: {:halt, false}
    end)
  end

  defp is_valid_draw?("blue", count) when count <= @blue, do: true
  defp is_valid_draw?("green", count) when count <= @green, do: true
  defp is_valid_draw?("red", count) when count <= @red, do: true
  defp is_valid_draw?(_, _), do: false
