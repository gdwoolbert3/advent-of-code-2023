defmodule AdventOfCode2023.Day2.Problem2 do
  @moduledoc """
  ...
  defp compute_power(draws) do
    acc = %{"blue" => 1, "green" => 1, "red" => 1}

    draws
    |> Enum.reduce(acc, fn {count, color}, acc ->
      Map.update!(acc, color, &max(&1, count))
    end)
    |> Map.values()
